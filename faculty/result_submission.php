...
?>
	...
	<div style="width:80%; margin:0px auto ;">
		...
		<div class="table-responsive">
			...
			<?php	
				$sql =  "SELECT * FROM marks_component m join comp c where m.comp_id=c.comp_id and m.assigned_course_id='$ass_course_id'";
...
if(isset($_POST['add_1'])){
	...
	$id = $_GET["value"];
	$sql ="SELECT * from res_process r join comp c where r.component_id=c.comp_id and c.component='$select_component' and r.assigned_course_id='$ass_course_id'"; 
	if($result = mysqli_query($connection,$sql)){
		if(mysqli_num_rows($result) > 0){
	...
	$sql2="SELECT s_id,name FROM st_en where assigned_course_id='$ass_course_id' and grade_id IS NULL";
		...
		$sql3="SELECT * FROM marks_component m join comp c WHERE m.comp_id=c.comp_id AND m.assigned_course_id='$ass_course_id' and c.component='$select_component'";
		if($result3=mysqli_query($connection,$sql3)){
			if(mysqli_num_rows($result3)>0){
				while ($row3 = mysqli_fetch_array($result3)){
...
if($result3=mysqli_query($connection,$sql3)){
	if(mysqli_num_rows($result3)>0){
		while ($row3 = mysqli_fetch_array($result3)){
		   $component=$row3['component']; 
		   $sql5="SELECT * FROM co WHERE assigned_course_id =  '$ass_course_id'";
		...
		if($result2=mysqli_query($connection,$sql2)){
			...
			$sql6="SELECT co_code
			 FROM co WHERE assigned_course_id='$ass_course_id'  ";
